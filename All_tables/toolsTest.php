...
class Tools{
  ...
  static function get_Tools_table(){
      try {
        connect();
            $stmt = $GLOBALS["conn"]->prepare("SELECT * from tools left join product on tools.product_product_id = product.product_id left join stock on product.stock_idstock = stock.idstock left join images on product.product_id = images.product_product_id
");
          $stmt->execute();
  ...
  static function get_Tools_tableByID($ID){
      try {
        connect();
            $stmt = $GLOBALS["conn"]->prepare("SELECT * from tools left join product on tools.product_product_id = product.product_id left join stock on product.stock_idstock = stock.idstock left join images on product.product_id = images.product_product_id where product_id = $ID
");
          $stmt->execute();
