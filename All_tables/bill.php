  ...
  $this->customer_idcustomer=$customer_idcustomer;
    ...
    public static function bill_table_constraints(){
        ...
        $stmt2=$GLOBALS["conn" ]->prepare(" ALTER TABLE Bill
                  ...
                  REFERENCES `newpro`.`customer` (`idcustomer`)
                    ON DELETE CASCADE
                    ON UPDATE CASCADE ");
...
static function get_Orders_Table(){
    try {
      connect();
          $stmt = $GLOBALS["conn"]->prepare("SELECT * from product join `bill line` on product.product_id = `bill line`.product_product_id join bill on `bill line`.bill_id_b = bill.id_b ");
        $stmt->execute();
